...
CREATE TABLE supply_usage (
  ...
  EmpId INT
    NOT NULL,
  SupplyUsageProdQtyAdded INT
    CHECK (SupplyUsageProdQtyAdded > 0)
    NOT NULL,
  SupplyUsageDate DATE
    DEFAULT CURRENT_DATE
    NOT NULL,
  SupplyUsageTime TIME
    DEFAULT CURRENT_TIME
    NOT NULL,
...
CREATE TABLE supply_usage_line_item (
  SupplyUsageLineItemId INT
    GENERATED BY DEFAULT AS IDENTITY
      (START WITH 1, INCREMENT BY 1)
    CHECK (SupplyUsageLineItemId >= 1)
    NOT NULL,
  SupplyUsageId INT
    NOT NULL,
  SupplyId INT
    NOT NULL,
  SupplyUsageLineItemQty DECIMAL(8, 2)
    CHECK (SupplyUsageLineItemQty > 0)
    NOT NULL,
  
  PRIMARY KEY (SupplyUsageLineItemId),
  FOREIGN KEY (SupplyUsageId)
    REFERENCES supply_usage (SupplyUsageId)
    ON UPDATE RESTRICT
    ON DELETE CASCADE,
  FOREIGN KEY (SupplyId)
    REFERENCES supplies (SupplyId)
...
CREATE TABLE supply_expiry (
  SupplyExpiryId INT
    GENERATED BY DEFAULT AS IDENTITY
      (START WITH 1, INCREMENT BY 1)
    CHECK (SupplyExpiryId >= 1)
    NOT NULL,
  SupplyId INT
    NOT NULL,
  SupplyExpiryQty DECIMAL(8, 2)
    CHECK (SupplyExpiryQty >= 0)
    NOT NULL,
  SupplyExpiryDate DATE
    NOT NULL,

  PRIMARY KEY (SupplyExpiryId),
  FOREIGN KEY (SupplyId)
    REFERENCES supplies (supplyid)
    ON UPDATE RESTRICT
    ON DELETE CASCADE
