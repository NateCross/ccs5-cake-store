...
Public Class Customer_Phone
    ...
    Public Overrides Sub RefreshDataGrid()
        ...
        Try
            ...
            SelectString = UtilityFunctions.Db2SelectStringGenerator("customer_phone", ColumnArray)
    ...
    Public Overloads Sub EventCreate(ByRef Values As List(Of String))
        ...
        Try
            StrInsert = UtilityFunctions.Db2InsertStringGenerator("customer_phone", ColumnArray, Values)
            ExecuteCommand(StrInsert)
            MsgBox("Successfully created customer phone entry.")
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    End Sub
    ...
    Public Overrides Sub EventDelete()
        ...
        Try
            StrDelete = UtilityFunctions.Db2DeleteStringGenerator("customer_phone", "custphoneid", ID)
            ExecuteCommand(StrDelete)
            MsgBox("Successfully deleted customer phone entry.")
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    End Sub
    ...
    Public Overloads Sub EventEdit(ByRef Values As List(Of String))
        Try
            Dim StrEdit As String = UtilityFunctions.Db2UpdateStringGenerator("customer_phone", ColumnArray, Values)
            ExecuteCommand(StrEdit)
            MsgBox("Successfully edited customer phone entry.")
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    End Sub
